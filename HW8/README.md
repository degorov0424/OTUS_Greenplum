...
#### 2. Создал MATERIALIZED VIEW dm.mv_sales_monthly
```sql
...
#### 3. Сравнил скорость выполнения запроса на основе VIEW и на основе витрины 
```sql
EXPLAIN ANALYZE
SELECT month_dt
     , customer_region
 ...
 */
 ...
 Запрос к view работает быстрее, но здесь не учитывается обновление view. Замерил и сравнил выполнение запросов с учетом обновления: 
...
END $$;
 ...
 Результат:
 ```txt
Время выполнения запроса к материализованному представлению: 00:00:00.689527
Время выполнения запроса к витрине: 00:00:00.193766
Общее время выполнения: 00:00:00.883293
 ```
В итоге время выполнения запроса с витриной, оказалось значительно меньше. 

---

#### 4. Написал запросы с использованием аналитических и оконных функций
 ```sql
SELECT c_name
